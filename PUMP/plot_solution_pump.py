...
# for k in range(3):
#     plt.subplot(3,3,k+1)
#     plt.plot(tgrid_x,np.concatenate(opt["Xd",:,:,'q',k]))
#     plt.plot(tgrid_x,np.concatenate(vars_init["Xd",:,:,'q',k]))
#     plt.title('q'+str(k))
#     plt.grid('on')
#     plt.subplot(3,3,k+4)
#     plt.plot(tgrid_x,np.concatenate(opt["Xd",:,:,'dq',k]))
#     plt.plot(tgrid_x,np.concatenate(vars_init["Xd",:,:,'dq',k]))
#     plt.title('dq'+str(k))
#     plt.grid('on')
#     plt.subplot(3,3,k+7)
#     plt.plot(tgrid_x,np.concatenate(opt["Xd",:,:,'w',k]))
#     plt.plot(tgrid_x,np.concatenate(vars_init["Xd",:,:,'w',k]))
#     plt.title('w'+str(k))
#     plt.grid('on')
# # --------------------------------------------------------
...
# for k in range(3):
#     plt.subplot(4,3,k+1)
#     plt.step(tgrid_u,[round(np.array(np.concatenate(opt['U',:,'u', k]))[o]) for o in range(0,nk)])
#     plt.step(tgrid_u,[round(np.array(np.concatenate(vars_init['U',:,'u', k]))[o]) for o in range(0,nk)])
#     plt.title('homotopy_accels_v'+str(k))
#     plt.grid('on')

#     plt.subplot(4,3,k+4)
#     plt.step(tgrid_u,[round(np.array(np.concatenate(opt['U',:,'T', k]))[o]) for o in range(0,nk)])
#     plt.step(tgrid_u,vars_init['U',:,'T', k])
#     plt.title('homotopy_accels_omega'+str(k))
#     plt.grid('on')

#     plt.subplot(4,3,k+7)
#     # plt.plot(tgrid_x,opt['Xd',:,:,'coeff',k].T*np.ones(tgrid_x.shape[0])*180/pi)
#     plt.plot(tgrid_x,np.concatenate(opt['Xd',:,:,'coeff',k]))
#     plt.plot(tgrid_x,np.concatenate(vars_init['Xd',:,:,'coeff',k]))
#     plt.title('coeff'+str(k))
#     plt.grid('on')

#     plt.subplot(4,3,k+10)
#     plt.step(tgrid_u,np.concatenate(opt['U',:,'dcoeff',k]))
#     plt.step(tgrid_u,np.concatenate(vars_init['U',:,'dcoeff',k]))
...
# ax.plot(np.concatenate(vars_init["Xd",:,:,'q',0]),np.concatenate(vars_init["Xd",:,:,'q',1]),np.concatenate(vars_init["Xd",:,:,'q',2]),'-')
# ax.scatter(0,0,0,'r')
...
# plt.plot(np.concatenate(vars_init["Xd",:,:,'q',0]),np.concatenate(vars_init["Xd",:,:,'q',1]),'-')
# plt.title('x-y')
# plt.grid('on')

# plt.subplot(2,2,3)
# plt.plot(np.concatenate(opt["Xd",:,:,'q',1]),np.concatenate(opt["Xd",:,:,'q',2]))
# plt.plot(np.concatenate(vars_init["Xd",:,:,'q',1]),np.concatenate(vars_init["Xd",:,:,'q',2]),'-')
# plt.title('y-z')
# plt.grid('on')

# plt.subplot(2,2,4)
# plt.plot(np.concatenate(opt["Xd",:,:,'q',0]),np.concatenate(opt["Xd",:,:,'q',2]),'-')
# plt.plot(np.concatenate(vars_init["Xd",:,:,'q',0]),np.concatenate(vars_init["Xd",:,:,'q',2]),'-')
# plt.title('x-z')
...
#                         np.sqrt(sum1(opt['Xd',k,j,'q']**2))))
# h2 = np.arange(5,1000,10)

# plt.figure()
# plt.subplot(2,2,1)
# plt.plot(tgrid_xa,val_opt['windspeed_shear'])
# # plt.plot(tgrid_x,params['wind0']*(np.array(opt["Xd",:,:,'q',2].T)/params['windShearRefAltitude'])**0.2)
# plt.grid('on')
...
#    a = 0.3*A
    ...
    # ax.plot(np.array(opt["Xd",k,:,'q',0]),
    #         np.array(opt["Xd",k,:,'q',1]),
    #         np.array(opt["Xd",k,:,'q',2]),linestyle = '--', color = 'grey',)
...
plt.subplot(2,2,2)
plt.plot(tgrid_x,np.concatenate(opt['Xd',:,:,'ltet']))
plt.plot(tgrid_x,np.concatenate(vars_init['Xd',:,:,'ltet']))
plt.grid('on')
plt.title('tether length')
plt.subplot(2,2,3)
plt.plot(tgrid_x,np.concatenate(opt['Xd',:,:,'dltet']))
plt.plot(tgrid_x,np.concatenate(vars_init['Xd',:,:,'dltet']))
plt.grid('on')
plt.title('tether speed')
plt.subplot(2,2,4)
plt.plot(tgrid_u,np.concatenate(opt['U',:,'ddltet']))
plt.plot(tgrid_u,np.concatenate(vars_init['U',:,'ddltet']))
plt.grid('on')
...
# fig = plt.figure()
# plt.subplot(1,2,1)
# plt.plot(np.array(opt["Xd",:,:,'q',0])[0],np.array(opt["Xd",:,:,'q',1])[0],'-')
...
#
# plt.subplot(1,2,2)
# plt.plot(np.array(opt["Xd",:,:,'q',0])[0],np.array(opt["Xd",:,:,'q',2])[0],'-')
...
# #        print j+3*i+1
#         plt.plot(tgrid_x,opt["Xd",:,:,"R",i+3*j].T)
#         plt.plot(tgrid_x,vars_init["Xd",:,:,"R",i+3*j].T)
#
